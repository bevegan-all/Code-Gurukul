  ...
  const SubjectCard = ({ s, type }) => {
    ...
    return (
      <button
        ...
        className={`w-full text-left bg-white rounded-2xl border shadow-sm p-5 hover:shadow-md transition-all group cursor-pointer ${
          isMajor ? 'border-gray-100 hover:border-blue-200' : 
          isVsc ? 'border-gray-100 hover:border-purple-200' :
        ...
        <div className="flex items-start justify-between mb-3">
          <div className={`p-2 rounded-xl ${
            ...
            <BookMarked className={`w-5 h-5 ${
              isMajor ? 'text-blue-600' : 
              isVsc ? 'text-purple-600' :
              'text-emerald-500'
            }`} />
          </div>
          <span className={`px-2 py-0.5 text-xs font-semibold rounded-full uppercase ${
            isMajor ? 'bg-blue-100 text-blue-700' : 
            isVsc ? 'bg-purple-100 text-purple-700' :
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      {subjects.length === 0 ? (
        ...
        <div className="space-y-8">
          {majors.length > 0 && (
            <div>
              <h2 className="text-sm font-black text-gray-400 uppercase tracking-[0.2em] mb-4">Major Subjects</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {majors.map(s => <SubjectCard key={s.subject_id} s={s} type="major" />)}
              </div>
            </div>
          )}
          ...
          {vscs.length > 0 && (
            <div>
              <h2 className="text-sm font-black text-gray-400 uppercase tracking-[0.2em] mb-4">VSC / Vocational Skill Courses</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {vscs.map(s => <SubjectCard key={s.subject_id} s={s} type="vsc" />)}
              </div>
            </div>
          )}
          ...
          {minors.length > 0 && (
            <div>
              <h2 className="text-sm font-black text-gray-400 uppercase tracking-[0.2em] mb-4">Minor / Elective Subjects</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {minors.map(s => <SubjectCard key={s.subject_id} s={s} type="minor" />)}
              </div>
            </div>
          )}
        </div>
