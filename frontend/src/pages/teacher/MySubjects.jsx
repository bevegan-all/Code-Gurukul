...
const MySubjects = () => {
  ...
  const vscs = subjects.filter(s => s.type === 'vsc');
  const minors = subjects.filter(s => s.type === 'minor');

  const SubjectCard = ({ s, type }) => {
    const isMajor = type === 'major';
    const isVsc = type === 'vsc';
    return (
      <button
        onClick={() => navigate(`/teacher/subjects/${s.subject_id}${s.class_id ? `?class_id=${s.class_id}` : ''}`)}
        className={`w-full text-left bg-white rounded-2xl border shadow-sm p-5 hover:shadow-md transition-all group cursor-pointer ${isMajor ? 'border-gray-100 hover:border-purple-200' : isVsc ? 'border-gray-100 hover:border-emerald-200' : 'border-gray-100 hover:border-blue-200'}`}
      >
        <div className="flex items-start justify-between mb-3">
          <div className={`p-2 rounded-xl ${isMajor ? 'bg-purple-50' : isVsc ? 'bg-emerald-50' : 'bg-blue-50'}`}>
            <BookMarked className={`w-5 h-5 ${isMajor ? 'text-purple-600' : isVsc ? 'text-emerald-600' : 'text-blue-500'}`} />
          </div>
          <span className={`px-2 py-0.5 text-xs font-semibold rounded-full ${isMajor ? 'bg-purple-100 text-purple-700' : isVsc ? 'bg-emerald-100 text-emerald-700' : 'bg-blue-100 text-blue-700'}`}>
            {type.toUpperCase()}
          </span>
        </div>
        ...
        <div className={`mt-3 flex items-center gap-1 text-xs font-semibold opacity-0 group-hover:opacity-100 transition-opacity ${isMajor ? 'text-purple-600' : isVsc ? 'text-emerald-600' : 'text-blue-600'}`}>
          ...
          {majors.length > 0 && (
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {majors.map((s, idx) => <SubjectCard key={`major-${idx}`} s={s} type="major" />)}
              </div>
            </div>
          )}
          {vscs.length > 0 && (
            <div>
              <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-3">VSC (Skill Courses)</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {vscs.map((s, idx) => <SubjectCard key={`vsc-${idx}`} s={s} type="vsc" />)}
              </div>
            </div>
          )}
          {minors.length > 0 && (
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {minors.map((s, idx) => <SubjectCard key={`minor-${idx}`} s={s} type="minor" />)}
              </div>
            </div>
          )}
        </>
